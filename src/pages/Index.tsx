...
import { RoomRejoiner } from '@/components/RoomRejoiner';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Brain, Sparkles, Trophy, Users, Gamepad2, MessageCircle, Crown } from 'lucide-react';

const Index = () => {
  const [user, setUser] = useState<any>(null);
  const [showLogin, setShowLogin] = useState(false);
  const [showRegister, setShowRegister] = useState(false);
  const [showRejoiner, setShowRejoiner] = useState(true);
  ...
  const handleNoRoomToRejoin = () => {
    setShowRejoiner(false);
  };

  // Show room rejoiner if user is logged in and we haven't checked for rooms yet
  if (user && showRejoiner) {
    return (
      <RoomRejoiner 
        user={user}
        onNoRoomToRejoin={handleNoRoomToRejoin}
        onLogout={handleLogout}
      />
    );
  }

  // Show dashboard if user is logged in and no room to rejoin
  if (user) {
