...
import { Brain, CheckCircle, Gift, XCircle, Zap } from 'lucide-react';
...
export const PuzzleGame = ({ user, onUpdateUser }: PuzzleGameProps) => {
  ...
  const [failedPuzzles, setFailedPuzzles] = useState<number[]>([]);
      ...
      for (const path of paths) {
        try {
          ...
          if (res.ok) {
            ...
            setPuzzles(parsed);
            setLoading(false);
            // After loading puzzles, set currentPuzzleIndex to first unsolved/unfailed puzzle
            setTimeout(() => {
  ...
  // Build the puzzle queue: unsolved/unfailed first, then failed, never solved
  const unsolvedUnfailed = puzzles.filter(p => !solvedPuzzles.includes(p.id) && !failedPuzzles.includes(p.id));
  const failed = puzzles.filter(p => failedPuzzles.includes(p.id) && !solvedPuzzles.includes(p.id));
  const puzzleQueue = [...unsolvedUnfailed, ...failed];
  const allSolved = puzzles.length > 0 && solvedPuzzles.length >= puzzles.length;
  const currentPuzzle = puzzleQueue[currentPuzzleIndex] || null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (correct && !solvedPuzzles.includes(currentPuzzle.id)) {
        ...
        // Remove from failedPuzzles if present
        const updatedFailedPuzzles = failedPuzzles.filter(id => id !== currentPuzzle.id);
        setFailedPuzzles(updatedFailedPuzzles);
        ...
        const newTimePlayed = (user.timePlayed || 0) + 1; // Assuming 1 minute per puzzle for now
        onUpdateUser({ coins: newCoins, experience: newExperience, puzzlesSolved: newPuzzlesSolved, level: newLevel, timePlayed: newTimePlayed });
        
        toast.success(`Correct! +${currentPuzzle.coins} coins, +${currentPuzzle.experience} XP`);
      } else if (!correct) {
        // Add to failedPuzzles if not already present
        if (!failedPuzzles.includes(currentPuzzle.id)) {
          setFailedPuzzles([...failedPuzzles, currentPuzzle.id]);
        }
        toast.error('Incorrect answer. The correct answer will be shown.');
      } else if (correct) {
  ...
  const nextPuzzle = () => {
    ...
    setIsCorrect(false);
    setUserAnswer('');
    // Move to next puzzle in the queue
    if (currentPuzzleIndex < puzzleQueue.length - 1) {
      setCurrentPuzzleIndex(currentPuzzleIndex + 1);
    } else {
      setCurrentPuzzleIndex(0); // fallback to start
    }
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-white flex items-center">
              ...
              {failedPuzzles.includes(currentPuzzle.id) && !solvedPuzzles.includes(currentPuzzle.id) && (
                <Badge variant="destructive" className="ml-2 text-xs">Retry</Badge>
              )}
            </CardTitle>
                ...
                <div>
                  ...
                  {!isCorrect && (
                    <div className="text-slate-300">
                      The correct answer is: <span className="font-semibold">{currentPuzzle.answer}</span>
                    </div>
                  )}
                  {isCorrect && solvedPuzzles.includes(currentPuzzle.id) && (
                    <div className="text-slate-300">You already solved this puzzle!</div>
                  )}
                </div>
              </div>
              <Button
