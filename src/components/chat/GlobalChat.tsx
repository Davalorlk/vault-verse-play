...
export const GlobalChat = ({ user }: GlobalChatProps) => {
  ...
  // Save chat state when component mounts
  useEffect(() => {
    // Import the saveGlobalChatState function
    import('@/lib/socket').then(({ saveGlobalChatState }) => {
      saveGlobalChatState(user);
    });
    // Restore chat messages from localStorage
    const stored = localStorage.getItem('mindVaultGlobalChatMessages');
    if (stored) {
      try {
  ...
  useEffect(() => {
    ...
    socket.on('chat-message', (msg) => {
      setMessages((prev) => {
        ...
        localStorage.setItem('mindVaultGlobalChatMessages', JSON.stringify(updated));
        return updated;
      });
    });
            ...
            {messages.map((msg) => (
              <div key={msg.id} className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-slate-300 break-words" dangerouslySetInnerHTML={{ __html: msg.message }}></p>
                </div>
              </div>
            ))}
