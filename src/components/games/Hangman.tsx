...
export function Hangman({ roomName, user, isMyTurn, playMode }: { roomName: string, user: any, isMyTurn: boolean, playMode: 'player' | 'computer' }) {
  ...
  useEffect(() => {
    if (playMode === 'player') {
      ...
      const handleGameState = (state: any) => {
        ...
        if (state.wrong !== undefined) {
          setWrong(state.wrong);
        }
        if (state.wordSet !== undefined) {
          setWordSet(state.wordSet);
        }
        if (state.round !== undefined) {
          setRound(state.round);
        }
        if (state.currentGuesser !== undefined) {
          setCurrentGuesser(state.currentGuesser);
        }
        if (state.currentWordSetter !== undefined) {
          setCurrentWordSetter(state.currentWordSetter);
        }
  ...
  useEffect(() => {
    if (wordSet && (won || lost) && playMode === 'player') {
      ...
      if (round >= 2) {
        ...
        socket.emit('game-state-update', {
          ...
          state: { 
            ...
            currentGuesser,
            currentWordSetter,
            winner: newScores[0] > newScores[1] ? 'Player 1' : newScores[1] > newScores[0] ? 'Player 2' : 'Draw',
            scores: newScores
          }
        });
      }
    }
  }, [won, lost, wordSet, round, currentGuesser, currentWordSetter, scores, playMode]);
    ...
    <div className="flex flex-col items-center justify-center w-full h-full px-3">
          ...
          {!gameStarted ? (
            ...
            <div className="space-y-2 p-3 bg-slate-800/50 rounded-lg">
              ...
              <div className="text-xs text-slate-400">
                Role: {isWordSetter ? 'Word Setter' : 'Guesser'}
              </div>
            </div>
          )}
        </div>
      ...
      <div className="w-full max-w-2xl bg-slate-800 rounded-lg p-6 shadow-xl">
        ...
        <div className="space-y-4">
            ...
            <div className="text-center">
              ...
              <div className="flex gap-2 justify-center">
                ...
                <button 
                  ...
                  disabled={!wordEntry.trim() || wordEntry.length < 2}
                >
                  Set Word
                </button>
              </div>
          ...
          {/* Waiting for word to be set */}
          {!wordSet && !isWordSetter && isMyTurnToPlay && (
            <div className="text-center">
              <div className="text-yellow-500 font-bold">
                Waiting for Player {currentWordSetter + 1} to set a word...
              </div>
            </div>
          )}
          ...
          {wordSet && !won && !lost && !isGuesser && isMyTurnToPlay && (
            <div className="text-center">
              <div className="text-yellow-500 font-bold">
                Waiting for Player {currentGuesser + 1} to make a guess...
              </div>
            </div>
          )}

          {/* Round End */}
          {wordSet && (won || lost) && !winner && (
            <div className="text-center">
